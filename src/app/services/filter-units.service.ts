...
export class FilterUnitsService {
  ...
  filterUnit(unit: Location, open_hour: string, close_hour: string) {
    if(!unit.schedules) return true

    let open_hour_filter = parseInt(open_hour, 10)
    let close_hour_filter = parseInt(close_hour, 10)
    let todays_weekdays = this.transformWeekday(new Date().getDay())

    for(let i = 0; i < unit.schedules.length; i++) {
      let schedule_hour = unit.schedules[i].hour
      let schedule_weekday = unit.schedules[i].weekdays

      if(todays_weekdays === schedule_weekday) {
        if(schedule_hour !== 'Fechada') {
          ...
          if(unit_open_hour_int <= open_hour_filter && unit_close_hour_int >= open_hour_filter) return true
          else false
        }
      }
    }
  ...
  filter(results:Location[], showClosed:boolean, hour:string):Location[] {
    ...
    if(hour){
      ...
      return intermediateResults.filter(location => this.filterUnit(location, OPEN_HOUR, CLOSE_HOUR))
    } else {
